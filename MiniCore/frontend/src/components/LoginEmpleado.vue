  ...
  export default {
    ...
    methods: {
        ...
        login() {
            ...
            api.post('http://localhost:8082/empleado/login', this.credentials)
            .then(response => {
                ...
                this.$router.push('/filtrar/'); // Redirigir a la página principal
            })
            .catch(error => {
                if (error.response) {
        ...
        register(){
            api.post('http://localhost:8082/empleado', this.newEmpleado)
            .then(() => {
